//  This is a V4L2 USB Skeleton Driver
...
MODULE_DESCRIPTION("V4L2 USB Skeleton Driver");
MODULE_AUTHOR("Abhijith S");
...
#define USB_SKEL_VENDOR_ID	0x13d3
#define USB_SKEL_PRODUCT_ID	0x56c9
...
static const struct usb_device_id skeleton_usb_tbl[] = {
	{ USB_DEVICE(USB_SKEL_VENDOR_ID, USB_SKEL_PRODUCT_ID) },
	{ }
};
MODULE_DEVICE_TABLE(usb, skeleton_usb_tbl);
...
{
	struct skeleton *skel = vb2_get_drv_priv(vq);  //return driver private data associated with the queue
...
// IOCTLS needs to be modified after defining methods..
/*
			     ...
			     struct v4l2_capability *cap)
{
	...
	strlcpy(cap->card, "V4L2 USB Skeleton", sizeof(cap->card));
	snprintf(cap->bus_info, sizeof(cap->bus_info), "usb:%s", skel->v4l2_dev.name);
	return 0;
}
...
static ssize_t pen_read(struct file *f, char __user *buf, size_t cnt, loff_t *off)
{
    ...
    /* Read the data frame from the bulk endpoint */
 ...
 */
static int skeleton_probe(struct usb_interface *pdev,const struct usb_device_id *id)
{
	struct skeleton *dev;
	struct video_device *vdev;
	struct vb2_queue *q;
    ...
    device = interface_to_usbdev(pdev);

    class.name = "usb/pen%d";
    class.fops = &pen_fops;
    if ((retval = usb_register_dev(pdev, &class)) < 0)
    {
	return retval;
    }
    else
	...
	dev = devm_kzalloc(&pdev->dev, sizeof(struct skeleton), GFP_KERNEL);
	if (!dev)
		return -ENOMEM;
	dev->pdev = pdev;
	...
	dev->timeperframe = (struct v4l2_fract) {1, 60};
	dev->width = 720;
	dev->height = 480;
	dev->pixelsize = 2;
	dev->input = 0;
...
static void skeleton_remove(struct usb_interface *pdev)
{
	struct skeleton *dev;
...
static struct usb_driver skeleton_driver = {
	.name = "skeleton",
	.id_table = skeleton_usb_tbl,
	.probe = skeleton_probe,
	.disconnect = skeleton_remove,
};
...
static int __init usb_init(void)
{
	...
	err = usb_register(&skeleton_driver);
	if(err)
...
static void __exit usb_exit(void)
{
	...
	usb_deregister(&skeleton_driver);
}
